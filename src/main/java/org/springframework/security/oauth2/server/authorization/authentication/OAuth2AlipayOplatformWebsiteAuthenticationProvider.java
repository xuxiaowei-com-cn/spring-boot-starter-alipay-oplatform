...
import org.springframework.security.oauth2.server.authorization.client.AlipayOplatformWebsiteTokenResponse;
import org.springframework.security.oauth2.server.authorization.client.RegisteredClient;
import org.springframework.security.oauth2.server.authorization.config.annotation.web.configurers.OAuth2AlipayOplatformWebsiteConfigurerUtils;
...
public class OAuth2AlipayOplatformWebsiteAuthenticationProvider implements AuthenticationProvider {
	...
	public OAuth2AlipayOplatformWebsiteAuthenticationProvider(HttpSecurity builder) {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		OAuth2AlipayOplatformWebsiteAuthenticationToken grantAuthenticationToken = (OAuth2AlipayOplatformWebsiteAuthenticationToken) authentication;
		...
		builder.principalName(userId);
		builder.authorizationGrantType(OAuth2AlipayOplatformWebsiteAuthenticationToken.ALIPAY_OPLATFORM_WEBSITE);

		AbstractAuthenticationToken abstractAuthenticationToken = alipayOplatformWebsiteService.authenticationToken(
				clientPrincipal, additionalParameters, grantAuthenticationToken.getDetails(), appid, code, userId,
				openId, null, unionId, accessToken, refreshToken, expiresIn);
		...
		DefaultOAuth2TokenContext.Builder tokenContextBuilder = DefaultOAuth2TokenContext.builder()
				...
				.authorizationGrantType(OAuth2AlipayOplatformWebsiteAuthenticationToken.ALIPAY_OPLATFORM_WEBSITE)
	...
	public boolean supports(Class<?> authentication) {
		return OAuth2AlipayOplatformWebsiteAuthenticationToken.class.isAssignableFrom(authentication);
	}
	...
	private void variableDefaults() {
		if (authorizationService == null) {
			authorizationService = OAuth2AlipayOplatformWebsiteConfigurerUtils.getAuthorizationService(builder);
		}

		if (tokenGenerator == null) {
			tokenGenerator = OAuth2AlipayOplatformWebsiteConfigurerUtils.getTokenGenerator(builder);
		}
